...
Proof.
  ...
  intros m n. induction n. 
    ...
    simpl. rewrite <- IHn. rewrite -> dist_succ_plus. 
    reflexivity.
Qed.
...
(* Similarly, the fact that [map] and [snoc] "commute" (in
   the sense that it doesn't matter which we do first)... *)
Lemma map_snoc : forall X Y : Set, 
                 forall f : X->Y,
                 forall v : X,
                 forall s : list X,
  map f (snoc s v) = snoc (map f s) (f v).
Proof.
...
Lemma snoc_with_append : forall X : Set, 
                         forall l1 l2 : list X,
                         forall v : X,
...
Proof.
  ...
  intros X l1 l2. induction l1.
    ...
    simpl. rewrite -> IHl1. rewrite -> snoc_with_append. reflexivity.
Qed.
...
Proof.
  ...
  intros X l. induction l.
    intro x. reflexivity.
    intro x0. simpl. rewrite <- IHl. reflexivity.
Qed.
