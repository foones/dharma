...
Proof.
  ...
  generalize dependent m.
  induction n.
    Case "0". intros m eq. destruct m.
      SCase "O". reflexivity.
      SCase "S". inversion eq. 
    Case "S". intros m eq. destruct m.
      SCase "O". inversion eq.
      SCase "S". assert (m = n) as H.
        SSCase "Proof of assertion". apply IHn. simpl in eq. 
          inversion eq. rewrite -> dist_succ_plus in eq. 
          rewrite -> dist_succ_plus in eq. inversion eq. 
          reflexivity.
        rewrite -> H. reflexivity.
Qed.
...
Proof.
  ...
  generalize dependent m.
  induction n.
    Case "O". intros m eq. destruct m.
      SCase "O". inversion eq.
      SCase "S". reflexivity.
    Case "S". intros m eq. destruct m.
      SCase "O". reflexivity.
      SCase "S". simpl. simpl in eq. apply eq.
Qed.
...
Proof.
  ...
  intros n X l. generalize dependent n. induction l.  
    ...
    Case "cons". intros n eq. simpl. simpl in eq. destruct n.
      SCase "O". inversion eq.
      SCase "S". inversion eq. rewrite -> H0. apply IHl. apply H0.
Qed.
Lemma length_append_cons : forall (X : Set) (l1 l2 : list X) 
                                  (x : X) (n : nat),
     length (l1 ++ (x :: l2)) = n
  -> S (length (l1 ++ l2)) = n.
Proof.
  (* Prove this by induction on [l1], without using
     [length_append]. *)
  (* SOLUTION *)
  intros X l1 l2 x. induction l1.
    Case "nil". intros n eq. simpl. simpl in eq. rewrite -> eq. reflexivity.
    Case "cons". simpl. intros n eq. destruct n.
      SCase "O". inversion eq.
      SCase "S". 
        assert (S (length (l1 ++ l2)) = n) as H.
          SSCase "Proof of assertion". apply IHl1. inversion eq. reflexivity.
        rewrite -> H. reflexivity.
Qed.
