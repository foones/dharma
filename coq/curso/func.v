...
Definition blt_nat (m n : nat) :=
  andb (ble_nat m n) (negb (beq_nat m n)).
...
Definition bge_nat (n m : nat) := ble_nat m n.
...
Definition PB_singleton (x : nat) : natlist := [x].
...
Fixpoint PB_filter (p : nat -> bool) (l : natlist) : natlist :=
  match l with
  | nil     => nil
  | x :: xs => if p x
                then x :: PB_filter p xs
                else PB_filter p xs
  end.

Definition PB_compose {a b c} (f : b -> c) (g : a -> b) (x : a) : c := f (g x).

(** Exercises *)
Definition nonzeros : natlist -> natlist :=
  PB_filter (fun (x : nat) => negb (beq_nat x 0)).

Lemma test_nonzeros:            nonzeros [0,1,0,2,3,0,0] = [1,2,3].
Proof. simpl. reflexivity. Qed.

Definition oddmembers : natlist -> natlist := PB_filter oddb.

Lemma test_oddmembers:            oddmembers [0,1,0,2,3,0,0] = [1,3].
Proof. simpl. reflexivity. Qed.

Definition countoddmembers : natlist -> nat := PB_compose length oddmembers.
...
Definition count (v:nat) : bag -> nat :=
  PB_compose length (PB_filter (beq_nat v)).
...
Definition add : nat -> bag -> bag := PB_compose app PB_singleton.
...
Fixpoint member (v:nat) (b:bag) : bool := 
  match b with
  | nil     => false 
  | x :: xs => orb (beq_nat x v) (member v xs)
  end.
...
Fixpoint remove_all (v:nat) : bag -> bag :=
  PB_filter (PB_compose negb (beq_nat v)).
...
Fixpoint doublematches (p: nat->bool) (l: natlist) {struct l} 
  ...
  match l with
  | nil => nil
  | x :: xs => (if p x then [x, x] else [x]) ++ doublematches p xs
  end.
...
(* Exercise *)
Definition countzeros : natlist -> nat := 
  PB_compose length (filter (beq_nat 0)).
...
Definition remove_onef (v:nat) (s:bagf) : bagf :=
  fun (x : nat) => if beq_nat v x then pred (s v) else (s x).
...
(* Exercise *)
Fixpoint repeat (X : Set) (x : X) (n : nat) {struct n} : list X := 
  match n with
  | O    => nil X
  | S n2 => cons X x (repeat X x n2)
  end.
...
Fixpoint reverse (X:Set) (l:list X) {struct l} : list X := 
  match l with
  | nil       => nil X
  | cons x xs => app X (reverse X xs) (cons X x (nil X))
  end.
