...
Proof.
  (* SOLUTION *)
  intros X R x y r.
  apply step with y. apply r. apply refl. 
Qed.
  ...
  apply step with
       (tm_plus
            ...
            (tm_plus
              (tm_const 2)
              (tm_const (plus 0 3)))).
  apply E_Plus2. apply v_const. apply E_Plus2.
        apply v_const. apply E_PlusConstConst.
  apply rsc_R.        
  apply E_Plus2. apply v_const. apply E_PlusConstConst.
Qed.
...
Proof.
  ...
  generalize dependent y2. induction P11; intros y2 P21 P22; inversion P21; subst.  
    auto. 
    impossible. apply P12. exists y. apply H. 
    impossible. apply P22. exists y. apply H. 
    apply IHP11; auto.
      assert (y = y0).  apply (step1_deterministic x); auto. 
    rewrite H2. auto. 
Qed.
  
(* Optional Exericse: If we defined [stepmany] in terms of [refl_transitive_closure]
rather than [refl_step_closure], proving this theorem is quite challenging.  Try it! *)
...
Proof.
  ...
  intros t1 t2 t2' Hv H. induction H.
    Case "rsc_refl". apply refl. 
    Case "rsc_step". 
      apply step with (tm_plus t1 y). 
        apply E_Plus2. apply Hv. apply H. 
        apply IHrefl_step_closure.
Qed.
...
Proof.
  (* SOLUTION *)
  intros t H.
  unfold normal_form. intros C.
  destruct C as [t' P]. apply H with t'. apply P.
Qed.
...
Proof.
  ...
  unfold normal_form.
  destruct H.
    Case "boolean value". inversion H.
      SCase "tm_true". intros Contra. destruct Contra as [t' P]. 
        inversion P.
      SCase "tm_false". intros Contra. destruct Contra as [t' P]. 
        inversion P.
    Case "numeric value".
      induction H.
        SCase "nv_zero". intros Contra. destruct Contra as [t' P]. 
          inversion P.
        SCase "nv_succ". intros Contra. destruct Contra as [t' P]. 
...
Proof.
  ...
  induction Hy1; intros y2 Hy2; inversion Hy2; subst; 
              try solve [auto | solve by inversion].
    Case "E_If".
      SCase "Hy2 by E_If". apply IHHy1 in H3. 
        subst. auto. 
    Case "E_Succ". apply IHHy1 in H0. subst. auto. 
    Case "E_PredSucc".
      SCase "Hy2 by E_Pred". inversion H1. subst.
        impossible. apply value_is_nf with t1. 
        unfold value. right. auto. 
        exists t1'0.  auto. 
    Case "E_Pred".
      SCase "Hy2 by E_PredSucc". inversion Hy1. subst.
        impossible. apply value_is_nf with y2. 
        unfold value. right. auto. 
        exists t1'0. auto. 
      SCase "Hy2 by E_Pred".   
        apply IHHy1 with (y2:=t1'0) in H0. 
        subst. auto. 
    Case "E_IszeroSucc".
      SCase "Hy2 by E_Iszero". inversion H1. subst.
        impossible. apply value_is_nf with t1. 
        unfold value. right. auto. 
        exists t1'0. auto. 
    Case "E_Iszero".
      SCase "Hy2 by E_IszeroSucc". inversion Hy1. subst.
        impossible. apply value_is_nf with t0. 
        unfold value. right. auto. 
        exists (t1'0). auto. 
      SCase "Hy2 by E_Iszero".
        apply IHHy1 with (y2:=t1'0) in H0.
        subst. auto.
Qed.
